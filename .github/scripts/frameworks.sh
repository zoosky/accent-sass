#!/usr/bin/env bash
#
# Compile a corpus of real-world CSS frameworks with grass and with dart-sass
...
# and ordering are reported but tolerated: they are grass's own long-standing
# behaviour, and canonicalising both sides (see NORMALISE below) shows they do
# not change what the stylesheet means.
#
# Usage: .github/scripts/frameworks.sh
#   GRASS      path to the grass binary        (default ./target/release/grass)
#   SASS       path to the dart-sass binary    (default ./dart-sass/sass)
#   NORMALISE  path to lightningcss, optional; when set, also reports the diff
#              after canonicalising both outputs through it
#   WORK       working directory               (default ./framework-corpus)

set -uo pipefail

GRASS=${GRASS:-./target/release/grass}
...
for entry in "${FRAMEWORKS[@]}"; do
  ...
  sass_args=()
  if [ "$loadpath" != "-" ]; then
    grass_args+=(-I "node_modules/$loadpath")
    sass_args+=("--load-path=node_modules/$loadpath")
  fi

  if ! "$GRASS" "${grass_args[@]}" "$input" > "$name-grass.css" 2>"$name-grass.err"; then
    echo "::error::grass failed to compile $name"
    head -n 20 "$name-grass.err"
    status=1
    continue
  fi
  "$SASS" --quiet "${sass_args[@]}" "$input" > "$name-sass.css" 2>/dev/null

  differing=$(diff "$name-grass.css" "$name-sass.css" | grep -c '^[<>]')
  colours=$(diff "$name-grass.css" "$name-sass.css" \
  ...
  normalised="n/a"
  if [ -n "$NORMALISE" ]; then
    if "$NORMALISE" --minify "$name-grass.css" 2>/dev/null | sed 's/}/}\n/g' > "$name-grass.norm" \
      && "$NORMALISE" --minify "$name-sass.css" 2>/dev/null | sed 's/}/}\n/g' > "$name-sass.norm" \
      && [ -s "$name-grass.norm" ] && [ -s "$name-sass.norm" ]; then
      normalised=$(diff "$name-grass.norm" "$name-sass.norm" | grep -c '^[<>]')
    else
  ...
  # configurations. dart-sass instantiates it once per configuration and grass
  # caches one instance per URL, so whole blocks legitimately differ; the
  ...
  if [ "$name" != "uswds" ] && [ "$colours" -ne 0 ]; then
    ...
    diff "$name-grass.css" "$name-sass.css" | grep '^[<>]' \
