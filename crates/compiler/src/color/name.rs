...
pub(crate) static NAMED_COLORS: NamedColorMap = NamedColorMap {
    ...
    rgba_to_name: phf::phf_map! {
        // phf infers the key type from the first entry and cannot see the
        // `[u8; 3]` on the field above, so that entry carries the suffix
        // explicitly. Since 0.14 the macro rejects an unsuffixed one rather
        // than defaulting it to `i32`.
